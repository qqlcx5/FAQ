  ...
  <div class="dialog-mobile">
    ...
    <div class="dlg-fd">
      <p class="tip">
        ...
        <el-button class="tip-size" size="mini" type="text" @click="switchPerson(1)">客服-2</el-button>
        <el-button class="tip-size" size="mini" type="text" @click="switchPerson(0)">客服-1</el-button>
  ...
  methods: {
    initSocket () {
      ...
      this.socket.on('join_resp', (data) =>{
        console.log("guest resp msg:"+data.message)
        let res= JSON.parse(data.message)//json转对象
        gb_username=res.u_name
        console.log("guest gb_username:"+gb_username)
        customer_list = res.customer_users
        console.log("guest customer_list:"+customer_list)
        let len= customer_list.length
        console.log("客服数:"+len)
        /*if(customer_list.length !=0){
            let customer_index = Math.ceil(Math.random()* customer_list.length)-1
            console.log("guest customer_index:"+ customer_index)
            gb_customer =customer_list[customer_index]
            console.log("guest gb_customer:"+gb_customer)
        }*/
      ...
      this.socket.on('connect', () => {
        ...
        let username= 'rock-'+ $formatDateTime(new Date());
        console.log("访客:"+username+"连接成功!");
        this.socket.emit('join', {username:username,user_type:'guest'});
      });
    ...
    sendQuestion (questionForm) {
      ...
      if (this.socket && this.socketStatus === 1) {
        this.socket.emit('send', JSON.stringify({user_type:'guest', type: 'personal', from: gb_username, to: gb_customer, msg: questionForm.question, time: $formatDateTime(new Date()) }));
        //this.socket.emit('client_request', JSON.stringify({type: 'personal', from: 'lockey', to: 'yxy', msg: questionForm.question, time: $formatDateTime(new Date()) }));
      }
    },
    clearChat () {
      this.chatLst = []
    ...
    switchPerson(index){
        gb_customer=customer_list[index]
        console.log("客服 switch to :"+ gb_customer)
    }
  },
  data () {
    return {
